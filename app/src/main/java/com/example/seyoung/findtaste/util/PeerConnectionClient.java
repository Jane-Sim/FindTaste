...
import com.example.seyoung.findtaste.util.AppRTCClient.SignalingParameters;
 ...
 * 피어 연결 클라이언트 구현.
 *
 * <p>모든 공용 메소드는 로컬 루퍼 스레드로 라우트됩니다.
...
public class PeerConnectionClient {
  ...
  // 대기중인 원격 ICE 응시자는 지역 및 원격 설명이 설정됩니다.
  // 로컬 ICE 후보자는 로컬 및 원격 설명이 모두 설정된 후에 원격 피어로 전송됩니다.
  private LinkedList<IceCandidate> queuedRemoteCandidates;
  private PeerConnectionEvents events;
  private boolean isInitiator;
  private SessionDescription localSdp; // SDP를 제안하거나 답변하십시오.
  private MediaStream mediaStream;
  private VideoCapturer videoCapturer;
  // enableVideo는 비디오를 렌더링하고 전송해야하는 경우 true로 설정됩니다.
  private boolean renderVideo;
  private VideoTrack localVideoTrack;
  private VideoTrack remoteVideoTrack;
  private RtpSender localVideoSender;
  // enableAudio는 오디오를 보내야하는 경우 true로 설정됩니다.
  ...
  public interface PeerConnectionEvents {
    /**
     * 로컬 SDP가 만들어지고 설정되면 콜백이 시작됩니다.
     */
    void onLocalDescription(final SessionDescription sdp);

    /**
     * 지역 Ice 후보가 생성되면 콜백이 시작됩니다.
     */
    void onIceCandidate(final IceCandidate candidate);

    /**
     * 콜백은 지역 ICE 후보자가 제거되면 해고됩니다.
     */
    void onIceCandidatesRemoved(final IceCandidate[] candidates);

    /**
     * 연결이 설정되면 콜백이 시작됩니다 (IceConnectionState는 CONNECTED).
     */
    void onIceConnected();

    /**
     * 연결이 닫히면 콜백이 시작됩니다 (IceConnectionState는 연결이 끊어졌습니다).
     */
    void onIceDisconnected();

    /**
     * 피어 연결이 닫히면 콜백이 시작됩니다.
     */
    void onPeerConnectionClosed();

    /**
     * 피어 연결 통계가 준비되면 콜백이 시작됩니다.
     */
    void onPeerConnectionStatsReady(final StatsReport[] reports);

    /**
     * 피어 연결 오류가 발생하면 콜백이 시작됩니다.
     */
    void onPeerConnectionError(final String description);
  }

  private PeerConnectionClient() {
    // executor 스레드는 privatector에서 한 번 시작되고 모든 피어 연결 API 호출에 사용되어
    // 새로운 피어 연결 팩토리가 이전에 파기 된 팩토리와 동일한 스레드에 작성되도록합니다.
  ...
  public void createPeerConnectionFactory(final Context context,
    ...
    localSdp = null; // SDP를 제안하거나 답변하십시오.
    mediaStream = null;
    videoCapturer = null;
    renderVideo = true;
    ...
    executor.execute(new Runnable() {
      ...
      public void run() {
        try {
          ...
          reportError("피어 연결을 생성하지 못했습니다.: " + e.getMessage());
          throw e;
        }
      }
    });
  ...
  private void createPeerConnectionFactoryInternal(Context context) {
    ...
    Log.d(TAG,
        "피어 연결 팩토리를 작성하십시오. 동영상 사용: " + peerConnectionParameters.videoCallEnabled);
    isError = false;
    ...
    // ISAC가 기본적으로 사용되는지 확인하십시오.
    preferIsac = peerConnectionParameters.audioCodec != null
        && peerConnectionParameters.audioCodec.equals(AUDIO_CODEC_ISAC);

    // OpenSL ES 재생을 활성화 / 비활성화합니다.
  ...
  private void createMediaConstraintsInternal() {
    // peer 연결 제약을 reate하십시오.
    pcConstraints = new MediaConstraints();
    // 일반 호출에는 DTLS를 사용하고 루프백 호출에는 사용하지 않도록 설정하십시오.
    ...
    // 기기에 카메라가 있는지 확인하고 그렇지 않은 경우 화상 통화를 사용 중지합니다.
    if (videoCapturer == null) {
      Log.w(TAG, "No camera on device. Switch to audio only call.");
      videoCallEnabled = false;
    }
    // 비디오 호출이 활성화 된 경우 비디오 제약 조건을 만듭니다.
    if (videoCallEnabled) {
      videoWidth = peerConnectionParameters.videoWidth;
      videoHeight = peerConnectionParameters.videoHeight;
      videoFps = peerConnectionParameters.videoFps;

      // 비디오 해상도가 지정되지 않은 경우 기본값은 HD입니다.
      if (videoWidth == 0 || videoHeight == 0) {
        videoWidth = HD_VIDEO_WIDTH;
        videoHeight = HD_VIDEO_HEIGHT;
      }

      // fps가 지정되지 않은 경우 기본값은 30입니다.
      if (videoFps == 0) {
        videoFps = 30;
      }
  ...
  private void createPeerConnectionInternal(EglBase.Context renderEGLContext) {
    ...
    // TCP 후보는 ICE-TCP를 지원하는 서버에 연결할 때만 유용합니다.
    ...
    // 기본 WebRTC 추적 및 INFO libjingle 로깅을 설정합니다.
  ...
  private static String setStartBitrate(
    ...
    // 이 코덱의 원격 SDP에 a = fmtp 문자열이 이미 있는지 확인하고 새 비트 전송률 매개 변수로 업데이트하십시오.
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      // 코덱에 해당 라인이없는 경우 new a = fmtp 라인을 추가하십시오.
  ...
  //"m = audio | video"를 포함한 행 번호를 돌려줍니다. 그러한 행이 존재하지 않는 경우는 -1을 돌려줍니다.
  ...
  private static String movePayloadTypesToFront(List<String> preferredPayloadTypes, String mLine) {
    ...
    // 미디어 설명 줄의 형식은 다음과 같아야합니다. m = <media> <port> <proto> <fmt> ...
    ...
    //preferredPayloadTypes |로 라인을 재구성하십시오. 페이로드 유형의 시작 부분으로 이동했습니다.
  ...
  private static String preferCodec(String sdpDescription, String codec, boolean isAudio) {
    ...
    //페이로드 유형은 96-127 범위의 정수이지만 여기에는 문자열로 저장됩니다.
  ...
  private void switchCameraInternal() {
    if (videoCapturer instanceof CameraVideoCapturer) {
      if (!videoCallEnabled || isError || videoCapturer == null) {
        ...
        return; // 비디오가 전송되지 않거나 하나의 카메라 만 사용 가능하거나 오류가 발생했습니다.
      }
  ...
  // 구현 세부 사항 : ICE 및 스트림 변경 사항을 관찰하고 이에 따라 대응하십시오.
  ...
  // 구현 세부 사항 : SDP가 설정되면 원격 ICE 후보자를 추가 할뿐만 아니라 오퍼 생성 / 신호 및 응답 설정을 처리합니다.
      ...
      executor.execute(new Runnable() {
        ...
        public void run() {
          ...
          if (isInitiator) {
            ...
            //피어 연결 제공을 위해 먼저 제안을 만들고 로컬 SDP를 설정 한 다음 응답 세트 SDP를받은 후
            if (peerConnection.getRemoteDescription() == null) {
              // We've just set our local SDP so time to send it.
              //우리는 로컬 SDP를 보낼 시간을 정했습니다.
              Log.d(TAG, "Local SDP set succesfully");
              events.onLocalDescription(localSdp);
            } else {
              // We've just set remote description, so drain remote
              // and send local ICE candidates.
              //우리는 원격 설명을 설정 했으므로 리모컨을 드레인하고 지역 ICE 후보자를 보냅니다.
            ...
            //또는 피어 연결에 응답하여 원격 SDP를 설정 한 다음 응답을 작성하고 로컬 SDP를 설정합니다.
            if (peerConnection.getLocalDescription() != null) {
              // We've just set our local SDP so time to send it, drain
              // remote and send local ICE candidates.
              //우리는 방금 현지 SDP를 보내 시간을 보내고 원격으로 유출하고 지역 ICE 후보자를 보냈습니다.
              Log.d(TAG, "Local SDP set succesfully");
              events.onLocalDescription(localSdp);
              drainCandidates();
            } else {
              ...
              //방금 원격 SDP를 설정했습니다. 지금은 아무 것도하지 마십시오. 곧 답변이 만들어집니다.
              Log.d(TAG, "Remote SDP set succesfully");
            }
          }
        }
