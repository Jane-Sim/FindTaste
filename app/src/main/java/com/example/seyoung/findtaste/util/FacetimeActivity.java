...
/**
 * Created by seyoung on 2018-01-20.
 */

public class FacetimeActivity  extends Activity {
    private static final String TAG = "FacetimeActivity";
    private static final int CONNECTION_REQUEST = 1;
    private static final int REMOVE_FAVORITE_INDEX = 0;
    private static boolean commandLineRun = false;

    private ImageView connectButton,userpic,chattime;
    private TextView username;
    public String roomEditText,userPic;
    ...
    private String activityTag = "FacetimeActivity";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        connectButton = findViewById(R.id.faceT);
        connectButton.setOnClickListener(connectListener);
        userpic = findViewById(R.id.userpic);
        chattime = findViewById(R.id.chattime);
        username = findViewById(R.id.username);
        final Intent intent = getIntent();
        //다른 사용자의 영상통화를 받았을 때 바로 영상통화 화면으로 넘어가게 한다.
...
//                Log.e("영상통화 상대방",friendname);
                connectToRoom(roomEditText, false, false, false, 0);
                finish();
            }
        }else { //사용자의 프로필사진을 눌렀을 때 받아올 정보들이다.

            //roomEditText = testMD5(inTime);
            friendid = intent.getStringExtra("friendid");
            friendname = intent.getStringExtra("friendname");
            friendpic = intent.getStringExtra("friendpic");
        ...
        Glide.with(this)                         //글라이드로 빠르게 사진을 넣는다.
                ...
                .apply(new RequestOptions()
                        ...
                        .circleCrop())
                .into(userpic);
      //  @SuppressLint("SimpleDateFormat") String time = new SimpleDateFormat("EEEMMMyyyyMMddHHmmsss").format(new Date(System.currentTimeMillis()));


        chattime.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                String time = String.valueOf(System.currentTimeMillis());
                CheckChattingList CC = new CheckChattingList();
                CC.execute(userid,friendid,time);
    ...
    //영상통화 방 이름을 암호화 시켜서 다른 유저가 접근하지 못하게 한다.
    ...
    //페이스타임을 서비스에 보낸다.
    private void sendFaceTimeToService(String facetime) {
        Log.e("바운드",mIsBound+"");
        if (mIsBound) {
            //    Log.e("서비스",mService.toString());
            if (mService != null) {
                try {
                    facetime = "/facetime"+"$"+userid+"$"+friendid+"$"+facetime+"$"+userPic+"$"+userN+"$";
                    Bundle b = new Bundle();
                    b.putString("facetime", facetime);
                    Log.e("진짜 방이름 보낸다",facetime);
                    Message msg = Message.obtain(null, MyService.MSG_SET_STRING_FACETIME);
                    msg.setData(b);
                    mService.send(msg);
                } catch (RemoteException e) {
                }
    ...
    public void connectToRoom(String roomId, boolean commandLineRun, boolean loopback,
        ...
        if (validateUrl(roomUrl)) {
            ...
            intent.putExtra(CallActivity.FRIENDNAME, friendname);
    ...
    private final View.OnClickListener connectListener = new View.OnClickListener() {
        @Override
        public void onClick(View view) {
            String inTime = new java.text.SimpleDateFormat("yyyyMMddHHmmsss", Locale.KOREA).format(new java.util.Date());
            roomEditText = testMD5(userid+friendid+inTime);
            connectToRoom(roomEditText, false, false, false, 0);
            handler.post(new Runnable() {
                public void run() {
                    sendFaceTimeToService(roomEditText);
                    Log.e("서버에서 보내는가?",roomEditText);
                }
            });
        }
    };
    ...
    class CheckChattingList extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String result) {
            if(result.equals("fail")){
                Toast.makeText(getApplicationContext(),"채팅방이 생성되지 않았습니다.",Toast.LENGTH_SHORT).show();
            }else {
                Intent intent = new Intent(getApplicationContext(), ChatRoomActivity.class);
                intent.putExtra("name", userN);
                intent.putExtra("friendid", friendid);
                intent.putExtra("friend", friendname);
                intent.putExtra("fImg", userPic);
